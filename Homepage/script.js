<script>
  ...
  // Display featured artwork
  async function displayFeaturedArtwork() {
    const visibleNFTs = totalNFTs.filter(n => !n.hidden);
    if (!visibleNFTs.length) return;

    const randomNFT = visibleNFTs[Math.floor(Math.random() * visibleNFTs.length)];
    const featuredContainer = document.getElementById('featured-artwork-container');
    document.getElementById('featured-title').textContent  = randomNFT.name;
    document.getElementById('featured-artist').textContent = randomNFT.creator;

    let mediaUrl       = optimizeImageUrl(randomNFT.imageUrl);
    let placeholderUrl = optimizeImageUrl(randomNFT.placeholder);

    placeholderUrl = normalizePlaceholderUrl(placeholderUrl);
    placeholderUrl = placeholderUrl.replace('/upload/', '/upload/w_600/');

    featuredContainer.innerHTML = await createMediaElement(mediaUrl, placeholderUrl, randomNFT.name);
  }
