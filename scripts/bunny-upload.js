...
function buildTitle(nftName, mediaUrl) {
  const trimmed = (nftName || '').trim();
  if (trimmed) return trimmed;
  if (mediaUrl) return path.basename(mediaUrl).slice(0, 120);
  return `NFT video ${Date.now()}`;
}
...
async function processNft(nft) {
  ...
  const title = buildTitle(nft.name, mediaUrl);
